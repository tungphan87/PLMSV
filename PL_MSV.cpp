            ...
            NumericVector p, NumericVector m, NumericVector v)
{
  ...
  mat theta_x_track = zeros(3,2*J);  // ------------------- qunatiles of theta_x = (alpha,beta)
  ...
  for(int t = 1 ; t < T; t++)
  {
    ...
    for(int i =0; i < N; i++)
    {
      ...
      //rho_part(i) = Sigma(0,1)/sqrt(Sigma(0,0)*Sigma(1,1));
    }
    ...
    // storing posterior 5%,50% and 97.5% 
    if(t == (T-1))
    {
      for(int j = 0; j < J; j++)
      {
